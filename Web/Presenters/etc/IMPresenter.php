...
final class IMPresenter
{
    ...
    public function load(array $params = []): void
    {
        ...
        if(isset($_COOKIE['zhabbler_session'])){
            ...
            $profile = (new User())->get_user_by_nickname($_POST['nickname']);
            $params += ["user" => $user, "profile" => $profile];
            $this->latte->render($_SERVER['DOCUMENT_ROOT']."/Web/etc/im.latte", $params);
        }else{
            die;
        }
    }
}
(new IMPresenter())->load();